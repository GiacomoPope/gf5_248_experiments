...
impl fp2::traits::Fq for GF5_248 {
    // Reexport constants for base field Trait
    const N: usize = 4;
    const ENCODED_LENGTH: usize = 32;
    const ZERO: Self = GF5_248::ZERO;
    const ONE: Self = GF5_248::ONE;
    const TWO: Self = GF5_248::TWO;
    const THREE: Self = GF5_248::THREE;
    const FOUR: Self = GF5_248::FOUR;
    const MINUS_ONE: Self = GF5_248::MINUS_ONE;
    ...
    fn select(a: &Self, b: &Self, ctl: u32) -> Self {
        GF5_248::select(a, b, ctl)
    }
    fn cond_swap(a: &mut Self, b: &mut Self, ctl: u32) {
        GF5_248::cond_swap(a, b, ctl)
    }
...
impl fp2::traits::FqRnd for GF5_248 {
...
mod tests {
    ...
    fn gf5_248_ops() {
        ...
        assert!(GF5_248::decode_reduce(&va).is_zero() == 0xFFFFFFFF);
        ...
        check_gf_ops(&va, &vb, &vx);
        assert!(GF5_248::decode_reduce(&va).is_zero() == 0);
        assert!(GF5_248::decode_reduce(&va).equals(&GF5_248::decode_reduce(&vb)) == 0xFFFFFFFF);
        for i in 0..4 {
    ...
    fn gf5_248_batch_invert() {
        ...
        for i in 0..300 {
            if xx[i].is_zero() != 0 {
                assert!(yy[i].is_zero() == 0xFFFFFFFF);
            } else {
