...
macro_rules! define_fp2_from_type {
    ...
    (@set_mul_products false, $Fp:ty, $self:expr, $other:expr) => {
        let _ = $other; // stop unused warning
        todo!()
    };
    ...
    (@set_square true, $Fp:ty, $self:expr) => {
        let x0 = <$Fp>::fp2_sq_re(&$self.x0, &$self.x1);
        let x1 = <$Fp>::fp2_sq_im(&$self.x0, &$self.x1);
        $self.x0 = x0;
        $self.x1 = x1;
    };
    (@set_square false, $Fp:ty, $self:expr) => {
    ...
    (@impl $typename:ident, $Fp:ty, $flag:tt) => {
        ...
        impl $typename {
            ...
            fn set_square(&mut self) {
                $crate::define_fp2_from_type!(@set_square $flag, $Fp, self);
            }
        ...
        impl fp2::traits::Fp2 for $typename {
            type BaseField = $Fp;
