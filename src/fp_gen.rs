...
macro_rules! define_fp_core {
    (
        ...
        impl $typename {
            ...
            const P_PLUS_ONE_LEADING_ZEROS: usize = Self::count_p_plus_one_leading_zeros();
            const MONTGOMERY_FRIENDLY: bool = Self::P_PLUS_ONE_LEADING_ZEROS > 0;
            const P_PLUS_ONE_LOW: u64 = Self::MODULUS[Self::P_PLUS_ONE_LEADING_ZEROS] + 1; // never overflows as this limb != u64::MAX
            ...
            const fn sum_of_products_check() -> bool {
                ...
                let (_, b1) = Self::addm(r1);

                // If the addition of the modulus has a carry, then return true
                b1 == 1
            }

            // A helper function which counts the number of leading limbs
            // of (MODULUS + 1) which are zero
            const fn count_p_plus_one_leading_zeros() -> usize {
                let mut count = 0;
                let mut i = 0;

                while i < Self::N {
                    if Self::MODULUS[i] == u64::MAX {
                        count += 1;
                    } else {
                        break;
                    }
        ...
        impl fp2::traits::Fq for $typename {
        ...
        impl fp2::traits::FqRoots for $typename {
        ...
        impl fp2::traits::FqExp for $typename {
        ...
        impl fp2::traits::FqRnd for $typename {
